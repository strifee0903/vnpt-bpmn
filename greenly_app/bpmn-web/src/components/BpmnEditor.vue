    ...
    <div class="row h-100">
      ...
      <div class="col-3 bg-light border-end overflow-auto py-3">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
          ...
          <button class="btn btn-warning mb-3" :disabled="!selectedProcessId"
            @click="updateProcess(selectedProcessId)">Update</button>
        </div>
        <ul class="list-group">
          <li class="list-group-item" :class="{ active: selectedProcessId === proc.process_id }"
            v-for="(proc, index) in processes" :key="index" @click="loadProcess(proc.process_id)"
      ...
      <div class="col-9 position-relative p-0">
        ...
        <div class="position-absolute top-0 end-50 m-3 z-3">
          <input v-model="processName" placeholder="Tên quy trình" class="form-control" style="width: 300px" />
        </div>
        ...
        <!-- Panel thuộc tính -->
        <div ref="propertiesPanelRef" class="position-absolute top-0 end-0 h-100 border-start bg-light overflow-auto"
...
const updateProcess = async (process_id) => {
  try {
    selectedProcessId.value = process_id
    ...
    const response = await fetch(`/api/v1/bpmn/${process_id}`, {
      method: 'PUT',
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error('Failed to update BPMN diagram: ' + errorText)
    }

    const result = await response.json() // Parse JSON response per Swagger spec
    console.log('BPMN process updated successfully:', result)
