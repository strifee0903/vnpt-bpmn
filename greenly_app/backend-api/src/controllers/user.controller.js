...
async function register(req, res, next) {
  ...
  try {
    ...
    return res
      ...
      .json(
        JSend.success({
          message: "The user has been registered! Please verify your email.",
          user: result.user,
        })
      );
  } catch (error) {
...
async function login(req, res, next) {
  ...
  try {
    ...
    if (!req.session) {
      return next(
        new ApiError(500, "Session middleware is not configured properly.")
      );
    }
    ...
    if (req.session.user && req.session.user.u_id) {
      return res.json(
        JSend.success({ message: "Already logged in!", user: req.session.user })
      );
    }
...
async function logout(req, res, next) {
  ...
  req.session.destroy((err) => {
    ...
    return res
      .status(200)
      .json(JSend.success({ message: "Sign out successfully!" }));
  });
}
...
async function checkAdminSession(req, res, next) {
  try {
    ...
    return res
      .status(200)
      .json(JSend.success({ message: "Admin session is valid." }));
  } catch (error) {
  ...
  try {
    ...
    if (!updated) {
      return next(
        new ApiError(
          400,
          `Unable to update the information of the user with the code ${id}`
        )
      );
    }
    return res.json(
      JSend.success({ message: "Updated successfully!", data: updated })
    );
  } catch (error) {
    console.log(error);
    return next(new ApiError(500, "System error, please try again later."));
...
async function deleteUser(req, res, next) {
  ...
  try {
    ...
    return res.json(
      JSend.success({ message: `Successfully deleted user ${id}` })
    );
  } catch (error) {
